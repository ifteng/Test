...
BOOL CBloodpressureDlg::OnInitDialog()
{
	...
	if (!hDll_NTPRx)
	{
		AfxMessageBox("加载RUIYIN_OMRON.dll失败");
		return FALSE;
	}

	InitializeTEST = (lpInitialize)GetProcAddress(hDll_NTPRx, "Initialize");
	if (NULL == InitializeTEST) { AfxMessageBox("找不到Initialize函数"); return FALSE; }

	SetCallBack = (lpSetCallBack)GetProcAddress(hDll_NTPRx, "SetCallBack");
	if (NULL == SetCallBack) { AfxMessageBox("找不到SetCallBack函数"); return FALSE; }
